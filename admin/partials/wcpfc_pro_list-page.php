...
$total_fees_per_page = get_option( 'chk_fees_per_page' );
$total_fees_per_page = ( $total_fees_per_page ) ? $total_fees_per_page : 10;
...
$get_all_fees_args  = array(
	...
	'order'          => 'DESC',
	'posts_per_page' => $total_fees_per_page,
	'orderby'        => 'ID',
	'paged'          => $get_paged,
);
...
$get_all_fees_pages = $get_all_fees_query->max_num_pages;
?>
	<div class="wcpfc-section-left">
		<?php if( !empty($getSuccess) && $getSuccess ) { ?>
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e('Your settings successfully saved.', 'woocommerce-conditional-product-fees-for-checkout'); ?></p>
			</div>
		<?php } ?>
			...
			<div class="product_header_title">
				<h2>
					...
					<?php if ( ! isset( $get_all_fees ) || $get_all_fees_count !== 0 ) { ?>
						...
						<a id="conditional-fee-order"
						   class="conditional-fee-order wcpfc-button"><?php esc_html_e( 'Save order', 'woocommerce-conditional-product-fees-for-checkout' ); ?></a>
					<?php } ?>
				</h2>
			</div>
			<div class="wcpfc-conditional-fee-listing-section">
				<table id="conditional-fee-listing" class="table-outer form-table conditional-fee-listing tablesorter">
					...
					<tr class="wcpfc-head">
						...
						<th class="sorter-false"><?php esc_html_e( 'Status', 'woocommerce-conditional-product-fees-for-checkout' ); ?></th>
							...
							<tr id="<?php echo esc_attr( $get_fees_id ); ?>"
								...
								<td>
									...
									<a class="fee-action-button button-primary"
									   ...
									   onclick="return confirm('Are you sure you would like to delete this fee?');"><?php esc_html_e( 'Delete', 'woocommerce-conditional-product-fees-for-checkout' ); ?></a>
		...
		$page_links = paginate_links(
			array(
				...
				'format'    => '',
				'prev_text' => __( '&laquo;', 'woocommerce-conditional-product-fees-for-checkout' ),
				'next_text' => __( '&raquo;', 'woocommerce-conditional-product-fees-for-checkout' ),
				'total'     => $get_all_fees_pages,
				'current'   => $get_paged,
			)
		);
		if ( $page_links ) {
