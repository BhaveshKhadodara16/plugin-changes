...
if( !class_exists( 'Woocommerce_Conditional_Product_Fees_For_Checkout_Pro' ) ){
	class Woocommerce_Conditional_Product_Fees_For_Checkout_Pro {
		...
		private function define_public_hooks() {
			...
			// $optional_fee_display_default_hook = apply_filters('optional_fee_display_default_hook', 'woocommerce_review_order_before_submit');
			if ( wcpffc_fs()->is__premium_only() ) {
				if ( wcpffc_fs()->can_use_premium_code() ) {
					// $this->loader->add_action( $optional_fee_display_default_hook, $plugin_public, 'add_option_to_checkout__premium_only' );
